<h1>Blog Artigos</h1>
<?= $this->Html->link('Adicionar Artigo', ['action' => 'adicionar']) ?>
<table>
    <tr>
        <th>Id</th>
        <th>Title</th>
        <th>Created</th>
        <th>Acoes</th>
    </tr>
    <!-- Aqui é onde iremos iterar nosso objeto de solicitação $articles,
    exibindo informações de artigos -->
    <?php foreach ($articles as $article): ?>
        <tr>
            <td><?= $article->id ?></td>
            <td><?= $this->Html->link($article->title, ['action' => 'view', $article->id]) ?></td>
            <td><?= $article->created->format(DATE_RFC850) ?></td>
            <td>
                <?=
                $this->Form->postLink(
                        'Deletar', ['action' => 'deletar', $article->id], ['confirm' => 'Tem certeza?'])
                ?>
                <?= $this->Html->link('Editar', ['action' => 'editar', $article->id]) ?></td>
        </tr>
    <?php endforeach; ?>
</table>
