...
1            <span class="ms in">Informação</span>
        </span>
        <div class="bloco list" style="display:block">
            ...
            <table width="1000" border="0" class="tbdados" style="float:left;" cellspacing="0" cellpadding="0">
                ...
                $Select = mysqli_query($_SG['link'],"SELECT * FROM clientes WHERE Nome LIKE '%$Busca%' AND IdUsuario = '$IdUsuario' ORDER BY Id DESC");
                while ($ResultadoSelect = mysqli_fetch_array($Select)) {
                    ?>
                    <?php
                    if (isset($_GET['Cliente'])) {
                        $Resgate = $_GET['Cliente'];
                        $Delete = mysqli_query($_SG['link'],"DELETE FROM clientes WHERE Identificador = '$Resgate'");
...
?>
            ...
            <div class="paginator">
                <?php
                $ListTotal = mysqli_query($_SG['link'],"SELECT Id FROM clientes WHERE IdUsuario = '$IdUsuario'");
